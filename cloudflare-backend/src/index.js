...
// ============================================
// Imported from assignment-questions.js
...
export default {
    async fetch(request, env, ctx) {
        ...
        try {
            ...
            if (path.match(/^\/api\/assignments\/\d+\/import-csv$/) && method === 'POST') {
                const assignmentId = path.split('/')[3];
                ...
                return await importAssignmentQuestionsCSV(env, request, assignmentId, csvData);
            }

            if (path.match(/^\/api\/assignments\/\d+\/export-csv$/) && method === 'GET') {
                const assignmentId = path.split('/')[3];
                return await exportAssignmentQuestionsCSV(env, request, assignmentId);
            }
