const express = require('express');
const cors = require('cors');
const Database = require('better-sqlite3');
const path = require('path');
const multer = require('multer');
const { parse } = require('csv-parse/sync');
const { stringify } = require('csv-stringify/sync');

const app = express();
app.use(cors());
app.use(express.json());

const upload = multer({ storage: multer.memoryStorage() });

// Initialize SQLite database
const db = new Database(path.join(__dirname, 'db.sqlite'));

// Create question_sets table
db.exec(`
  CREATE TABLE IF NOT EXISTS question_sets (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    name TEXT NOT NULL,
    description TEXT,
    showInstantFeedback INTEGER DEFAULT 0,
    presentationMode INTEGER DEFAULT 0,
    timePerQuestion INTEGER DEFAULT 30,
    shuffleQuestions INTEGER DEFAULT 1,
    shuffleChoices INTEGER DEFAULT 0,
    allowSkip INTEGER DEFAULT 1,
    showScore INTEGER DEFAULT 1,
    createdAt INTEGER DEFAULT (strftime('%s', 'now'))
  )
`);

// Create questions table with setId
db.exec(`
  CREATE TABLE IF NOT EXISTS questions (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    setId INTEGER DEFAULT 1,
    text TEXT NOT NULL,
    choice1 TEXT NOT NULL,
    choice2 TEXT NOT NULL,
    choice3 TEXT NOT NULL,
    choice4 TEXT NOT NULL,
    correctIndex INTEGER NOT NULL,
    explanation TEXT,
    createdAt INTEGER DEFAULT (strftime('%s', 'now')),
    FOREIGN KEY (setId) REFERENCES question_sets(id) ON DELETE CASCADE
  )
`);

// Seed initial data if empty
const setCount = db.prepare('SELECT COUNT(*) as count FROM question_sets').get();
if (setCount.count === 0) {
  const insertSet = db.prepare(`
    INSERT INTO question_sets (name, description, showInstantFeedback, presentationMode, timePerQuestion)
    VALUES (?, ?, ?, ?, ?)
  `);
  
  insertSet.run('Toán học cơ bản', 'Các phép tính đơn giản', 0, 0, 30);
  insertSet.run('Khoa học tự nhiên', 'Động vật và thiên nhiên', 1, 0, 45);
  insertSet.run('Địa lý Việt Nam', 'Kiến thức về Việt Nam', 0, 1, 60);
  
  console.log('✅ Database seeded with question sets');
}

const questionCount = db.prepare('SELECT COUNT(*) as count FROM questions').get();
if (questionCount.count === 0) {
  const insert = db.prepare(`
    INSERT INTO questions (setId, text, choice1, choice2, choice3, choice4, correctIndex, explanation)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `);

  const seedData = [
    // Set 1: Toán học
    [1, '2 + 3 bằng bao nhiêu?', '4', '5', '6', '3', 1, '2 cộng 3 bằng 5'],
    [1, '5 - 2 bằng bao nhiêu?', '2', '3', '4', '5', 1, '5 trừ 2 bằng 3'],
    [1, '3 × 2 bằng bao nhiêu?', '5', '6', '7', '8', 1, '3 nhân 2 bằng 6'],
    // Set 2: Khoa học
    [2, 'Con vật nào kêu "meo meo"?', 'Chó', 'Mèo', 'Gà', 'Vịt', 1, 'Mèo kêu meo meo'],
    [2, 'Con gà có mấy chân?', '1', '2', '3', '4', 1, 'Gà có 2 chân'],
    [2, 'Màu của lá cây là gì?', 'Đỏ', 'Vàng', 'Xanh', 'Trắng', 2, 'Lá cây có màu xanh'],
    [2, 'Con nào biết bay?', 'Cá', 'Chó', 'Chim', 'Chuột', 2, 'Chim biết bay'],
    // Set 3: Địa lý
    [3, 'Thủ đô của Việt Nam là gì?', 'TP.HCM', 'Hà Nội', 'Đà Nẵng', 'Cần Thơ', 1, 'Thủ đô là Hà Nội'],
    [3, 'Mặt trời mọc ở hướng nào?', 'Tây', 'Bắc', 'Đông', 'Nam', 2, 'Mặt trời mọc ở hướng Đông'],
    [3, '1 tuần có bao nhiêu ngày?', '5', '6', '7', '8', 2, '1 tuần có 7 ngày']
  ];

  seedData.forEach(data => insert.run(...data));
  console.log('✅ Database seeded with initial questions');
}

// ============================================
// QUESTION SETS APIs
// ============================================

// Get all question sets
app.get('/api/sets', (req, res) => {
  try {
    const sets = db.prepare(`
      SELECT s.*, 
        (SELECT COUNT(*) FROM questions WHERE setId = s.id) as questionCount
      FROM question_sets s 
      ORDER BY id
    `).all();
    res.json(sets);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

// Get single set
app.get('/api/sets/:id', (req, res) => {
  try {
    const set = db.prepare('SELECT * FROM question_sets WHERE id = ?').get(req.params.id);
    if (!set) return res.status(404).json({ error: 'Not found' });
    res.json(set);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

// Create question set
app.post('/api/sets', (req, res) => {
  try {
    const { name, description, showInstantFeedback, presentationMode, timePerQuestion, shuffleQuestions, shuffleChoices, allowSkip, showScore } = req.body;
    
    const insert = db.prepare(`
      INSERT INTO question_sets 
      (name, description, showInstantFeedback, presentationMode, timePerQuestion, shuffleQuestions, shuffleChoices, allowSkip, showScore)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
    
    const result = insert.run(
      name, 
      description || '', 
      showInstantFeedback ? 1 : 0,
      presentationMode ? 1 : 0,
      timePerQuestion || 30,
      shuffleQuestions !== false ? 1 : 0,
      shuffleChoices ? 1 : 0,
      allowSkip !== false ? 1 : 0,
      showScore !== false ? 1 : 0
    );
    
    const newSet = db.prepare('SELECT * FROM question_sets WHERE id = ?').get(result.lastInsertRowid);
    res.status(201).json(newSet);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

// Update question set
app.put('/api/sets/:id', (req, res) => {
  try {
    const { name, description, showInstantFeedback, presentationMode, timePerQuestion, shuffleQuestions, shuffleChoices, allowSkip, showScore } = req.body;
    
    const update = db.prepare(`
      UPDATE question_sets 
      SET name = ?, description = ?, showInstantFeedback = ?, presentationMode = ?, 
          timePerQuestion = ?, shuffleQuestions = ?, shuffleChoices = ?, allowSkip = ?, showScore = ?
      WHERE id = ?
    `);
    
    update.run(
      name,
      description || '',
      showInstantFeedback ? 1 : 0,
      presentationMode ? 1 : 0,
      timePerQuestion || 30,
      shuffleQuestions !== false ? 1 : 0,
      shuffleChoices ? 1 : 0,
      allowSkip !== false ? 1 : 0,
      showScore !== false ? 1 : 0,
      req.params.id
    );
    
    const updated = db.prepare('SELECT * FROM question_sets WHERE id = ?').get(req.params.id);
    res.json(updated);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

// Delete question set
app.delete('/api/sets/:id', (req, res) => {
  try {
    db.prepare('DELETE FROM question_sets WHERE id = ?').run(req.params.id);
    res.json({ success: true });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

// ============================================
// QUESTIONS APIs
// ============================================

// Get all questions (optionally filter by setId)
app.get('/api/questions', (req, res) => {
  try {
    const { setId } = req.query;
    let query = 'SELECT * FROM questions';
    let params = [];
    
    if (setId) {
      query += ' WHERE setId = ?';
      params.push(setId);
    }
    
    query += ' ORDER BY id DESC';
    
    const questions = db.prepare(query).all(...params);
    res.json(questions.map(q => ({
      id: q.id,
      setId: q.setId,
      text: q.text,
      choices: [q.choice1, q.choice2, q.choice3, q.choice4],
      correctIndex: q.correctIndex,
      explanation: q.explanation,
      createdAt: q.createdAt
    })));
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

// Get single question
app.get('/api/questions/:id', (req, res) => {
  try {
    const q = db.prepare('SELECT * FROM questions WHERE id = ?').get(req.params.id);
    if (!q) return res.status(404).json({ error: 'Not found' });
    res.json({
      id: q.id,
      setId: q.setId,
      text: q.text,
      choices: [q.choice1, q.choice2, q.choice3, q.choice4],
      correctIndex: q.correctIndex,
      explanation: q.explanation
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

// Create question
app.post('/api/questions', (req, res) => {
  try {
    const { setId, text, choices, correctIndex, explanation } = req.body;
    if (!text || !choices || choices.length < 4 || correctIndex === undefined) {
      return res.status(400).json({ error: 'Invalid data' });
    }
    
    const insert = db.prepare(`
      INSERT INTO questions (setId, text, choice1, choice2, choice3, choice4, correctIndex, explanation)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `);
    
    const result = insert.run(
      setId || 1,
      text, 
      choices[0], 
      choices[1], 
      choices[2], 
      choices[3], 
      correctIndex,
      explanation || ''
    );
    
    const newQuestion = db.prepare('SELECT * FROM questions WHERE id = ?').get(result.lastInsertRowid);
    
    res.status(201).json({
      id: newQuestion.id,
      setId: newQuestion.setId,
      text: newQuestion.text,
      choices: [newQuestion.choice1, newQuestion.choice2, newQuestion.choice3, newQuestion.choice4],
      correctIndex: newQuestion.correctIndex,
      explanation: newQuestion.explanation
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

// Update question
app.put('/api/questions/:id', (req, res) => {
  try {
    const { setId, text, choices, correctIndex, explanation } = req.body;
    const update = db.prepare(`
      UPDATE questions 
      SET setId = ?, text = ?, choice1 = ?, choice2 = ?, choice3 = ?, choice4 = ?, correctIndex = ?, explanation = ?
      WHERE id = ?
    `);
    
    update.run(
      setId || 1,
      text, 
      choices[0], 
      choices[1], 
      choices[2], 
      choices[3], 
      correctIndex,
      explanation || '',
      req.params.id
    );
    
    const updated = db.prepare('SELECT * FROM questions WHERE id = ?').get(req.params.id);
    res.json({
      id: updated.id,
      setId: updated.setId,
      text: updated.text,
      choices: [updated.choice1, updated.choice2, updated.choice3, updated.choice4],
      correctIndex: updated.correctIndex,
      explanation: updated.explanation
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

// Delete question
app.delete('/api/questions/:id', (req, res) => {
  try {
    db.prepare('DELETE FROM questions WHERE id = ?').run(req.params.id);
    res.json({ success: true });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

// ============================================
// QUIZ APIs
// ============================================

// Get a quiz from a specific set
app.get('/api/quiz', (req, res) => {
  try {
    const { setId, count = 5 } = req.query;
    
    let query = 'SELECT * FROM questions';
    let params = [];
    
    if (setId) {
      query += ' WHERE setId = ?';
      params.push(setId);
    }
    
    query += ' ORDER BY RANDOM() LIMIT ?';
    params.push(parseInt(count, 10));
    
    const questions = db.prepare(query).all(...params);
    
    // Get set settings if setId provided
    let setSettings = null;
    if (setId) {
      setSettings = db.prepare('SELECT * FROM question_sets WHERE id = ?').get(setId);
    }
    
    res.json({
      setSettings,
      questions: questions.map(q => ({
        id: q.id,
        text: q.text,
        choices: [q.choice1, q.choice2, q.choice3, q.choice4],
        explanation: q.explanation
        // Don't send correctIndex to client!
      }))
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

// Grade quiz
app.post('/api/grade', (req, res) => {
  try {
    const { answers } = req.body; // [{ id, answerIndex }]
    if (!answers || !Array.isArray(answers)) {
      return res.status(400).json({ error: 'Invalid data' });
    }
    
    let correct = 0;
    const details = answers.map(a => {
      const q = db.prepare('SELECT * FROM questions WHERE id = ?').get(a.id);
      if (!q) return { id: a.id, isCorrect: false };
      
      const isCorrect = q.correctIndex === a.answerIndex;
      if (isCorrect) correct++;
      
      return {
        id: a.id,
        questionText: q.text,
        correctIndex: q.correctIndex,
        correctAnswer: [q.choice1, q.choice2, q.choice3, q.choice4][q.correctIndex],
        yourAnswer: [q.choice1, q.choice2, q.choice3, q.choice4][a.answerIndex] || 'Chưa chọn',
        yourAnswerIndex: a.answerIndex,
        isCorrect,
        explanation: q.explanation
      };
    });
    
    res.json({
      total: answers.length,
      correct,
      score: Math.round((correct / answers.length) * 100),
      details
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

// Check single answer (for instant feedback mode)
app.post('/api/check-answer', (req, res) => {
  try {
    const { questionId, answerIndex } = req.body;
    const q = db.prepare('SELECT * FROM questions WHERE id = ?').get(questionId);
    
    if (!q) return res.status(404).json({ error: 'Question not found' });
    
    const isCorrect = q.correctIndex === answerIndex;
    
    res.json({
      isCorrect,
      correctIndex: q.correctIndex,
      correctAnswer: [q.choice1, q.choice2, q.choice3, q.choice4][q.correctIndex],
      explanation: q.explanation
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

// ============================================
// CSV Import/Export
// ============================================

// Import CSV
app.post('/api/import-csv', upload.single('file'), (req, res) => {
  try {
    if (!req.file) return res.status(400).json({ error: 'No file uploaded' });
    
    const { setId } = req.body;
    const content = req.file.buffer.toString('utf-8');
    const records = parse(content, {
      columns: true,
      skip_empty_lines: true,
      bom: true
    });
    
    const insert = db.prepare(`
      INSERT INTO questions (setId, text, choice1, choice2, choice3, choice4, correctIndex, explanation)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `);
    
    let imported = 0;
    for (const record of records) {
      if (record.question && record.choice1 && record.correctIndex !== undefined) {
        insert.run(
          setId || 1,
          record.question,
          record.choice1 || '',
          record.choice2 || '',
          record.choice3 || '',
          record.choice4 || '',
          parseInt(record.correctIndex, 10),
          record.explanation || ''
        );
        imported++;
      }
    }
    
    res.json({ success: true, imported });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

// Export CSV
app.get('/api/export-csv', (req, res) => {
  try {
    const { setId } = req.query;
    let query = 'SELECT * FROM questions';
    let params = [];
    
    if (setId) {
      query += ' WHERE setId = ?';
      params.push(setId);
    }
    
    const questions = db.prepare(query).all(...params);
    const records = questions.map(q => ({
      question: q.text,
      choice1: q.choice1,
      choice2: q.choice2,
      choice3: q.choice3,
      choice4: q.choice4,
      correctIndex: q.correctIndex,
      explanation: q.explanation || ''
    }));
    
    const csv = stringify(records, {
      header: true,
      columns: ['question', 'choice1', 'choice2', 'choice3', 'choice4', 'correctIndex', 'explanation']
    });
    
    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
    res.setHeader('Content-Disposition', 'attachment; filename=questions.csv');
    res.send('\uFEFF' + csv); // BOM for UTF-8
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

const PORT = process.env.PORT || 4000;
app.listen(PORT, () => {
  console.log(`🚀 Backend server running at http://localhost:${PORT}`);
  console.log(`📊 Database: ${path.join(__dirname, 'db.sqlite')}`);
});

// Graceful shutdown
process.on('SIGINT', () => {
  db.close();
  process.exit(0);
});
