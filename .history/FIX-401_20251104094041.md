# 🔧 Fix 401 Unauthorized Error

## Vấn đề
- API trả về 401 khi gọi `/api/assignments`
- Lỗi: Token không được gửi hoặc không hợp lệ

## ✅ Đã fix

### 1. API Interceptor (api.js)
Thêm interceptor tự động lấy token từ localStorage mỗi request:

```javascript
api.interceptors.request.use((config) => {
    const token = localStorage.getItem('token')
    if (token) {
        config.headers.Authorization = `Bearer ${token}`
    }
    return config
})
```

### 2. Auto redirect on 401
```javascript
api.interceptors.response.use(
    (response) => response,
    (error) => {
        if (error.response?.status === 401) {
            localStorage.removeItem('token')
            localStorage.removeItem('user')
            window.location.href = '/login'
        }
        return Promise.reject(error)
    }
)
```

## 🧪 Cách test

### Bước 1: Clear và Login lại

Mở DevTools Console (F12) và chạy:
```javascript
// Clear storage
localStorage.clear()

// Reload
location.reload()
```

### Bước 2: Login với teacher1
1. Click "Đăng nhập"
2. Click button quick login "teacher1"
3. Check console log

### Bước 3: Test API
Truy cập: http://localhost:5173/test-api

Click các buttons:
- ✅ Check localStorage - xem token có được lưu không
- ✅ Test Health - test endpoint public
- ✅ Test Profile - test endpoint có auth
- ✅ Test Assignments - test endpoint assignments

### Bước 4: Check Network tab
1. Mở DevTools > Network tab
2. Reload trang /teacher/assignments
3. Click vào request `/api/assignments`
4. Check **Headers** section:
   - Phải có: `Authorization: Bearer <token>`

## 🐛 Nếu vẫn lỗi

### Check 1: Token format
Mở Console:
```javascript
const token = localStorage.getItem('token')
console.log('Token:', token)
console.log('Token starts with:', token.substring(0, 20))
```

Token phải có dạng: `eyJ1c2VySWQiOjEsInJ...`

### Check 2: Backend verification
Test API trực tiếp với curl:

```powershell
# Login
$body = '{"username":"teacher1","password":"password123"}'
$response = Invoke-RestMethod -Method POST -Uri "https://quiz-game-api.quiz-game-khanhan.workers.dev/api/auth/login" -ContentType "application/json" -Body $body
$token = $response.token

# Test assignments
$headers = @{ "Authorization" = "Bearer $token" }
Invoke-RestMethod -Method GET -Uri "https://quiz-game-api.quiz-game-khanhan.workers.dev/api/assignments" -Headers $headers
```

### Check 3: Token expiry
Token có thời hạn 7 ngày. Nếu quá hạn phải login lại.

## 📝 Files đã sửa
- ✅ `frontend/src/api.js` - Added interceptors
- ✅ `frontend/src/pages/TestAPI.jsx` - Debug page
- ✅ `frontend/src/App.jsx` - Added /test-api route

## 🎯 Next Steps
1. Clear localStorage
2. Login lại
3. Test /teacher/assignments
4. Nếu OK → Import sample assignments
5. Test full flow!
